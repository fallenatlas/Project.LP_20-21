...
espaco_fila([P | R], Esp, H_V) :-
    P == [X|Y], espaco_fila(R, Esp, H_V).
espaco_fila([P | R], [P | Esp], H_V) :-
    P \== [X|Y], espaco_fila(R, Esp, H_V).


espaco_fila(Fila, Esp, H_V) :-
    bagof(E, (member(E, Fila), \+ is_list(E)), Esp).
*/
...
find_empty([], Esp, [], [])
find_empty([P|R], Esp, [P|List_esp], List) :-
    \+ is_list(P), List_esp == [].
...
find_empty([P|R], Esp, List_esp, List) :-
    is_list(P), List_esp == [], find_empty(R, Esp, List_esp, List), nth1(1, P, Soma), Esp = espaco(Soma, List).

find_empty([P|R], Esp, List, List) :-
*/
...
permutacao_possivel_espaco(Perm, Esp, Espacos, Perms_soma) :-
    ...
    %write_ln("espacos e perms em comum"),
    %write_ln(Perms_soma_com),
    perms_possiveis(Espaco, Perms_Esp, Perms_soma_com, Perm).
...
perms_possiveis(Espaco, [Perm|R], Perms_soma_com, Perm) :-
    compativeis(Espaco, Perm, Perms_soma_com).
...
compativeis([Var|R1], [Num|R2], [Esp_perm_com|R3]) :-
    ...
    nth1(1, N_var, N),
    %write_ln(N),
    bagof(Perm, (member(Perm, Perms_poss), nth1(N, Perm, Num)), Comp),
    %write_ln(Comp),
    %write_ln("here"),
    compativeis(R1, R2, R3).
...
in_perms(Espaco, Espacos, Outro_perms, Perm, Elem, Possiveis) :-
    %search_var_pos(Var, [Var|R], N).
    nth1(N, Perm, Elem),
    nth1(N, Espaco, Elem),
    bagof(Perms, (membro(Perms, Outro_perms), subsumes_term(Espaco, Perms)), Possiveis).
